mongo -u $MONGO_INITDB_ROOT_USERNAME -p mongo-pw admin --eval "rs.initiate();"
mongo -u $MONGO_INITDB_ROOT_USERNAME -p mongo-pw admin --eval 'db = db.getSiblingDB("admin");
db.auth("mongo-user", "admin")
db.createUser({
  "user": "datadog",
  "pwd": "datadog",
  "roles": [
...
mongo -u $MONGO_INITDB_ROOT_USERNAME -p mongo-pw admin --eval "db = db.getSiblingDB('config'); db.createRole({
    role: 'dbz-role',
    privileges: [
        {
...
mongo -u $MONGO_INITDB_ROOT_USERNAME -p mongo-pw admin --eval 'db = db.getSiblingDB("admin"); db.createUser({
  "user" : "dbz-user",
  "pwd": "dbz-pw",
  "roles" : [
    {
...
mongo -u $MONGO_INITDB_ROOT_USERNAME -p mongo-pw admin --eval 'db = db.getSiblingDB("logistics");
